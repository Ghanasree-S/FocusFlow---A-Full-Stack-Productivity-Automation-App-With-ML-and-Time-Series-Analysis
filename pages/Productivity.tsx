...
import { Play, Pause, RotateCcw, Check, Flag, Calendar, Trash2, Edit2, Volume2, VolumeX, X } from 'lucide-react';
import { Card, Button, Badge } from '../components/UI';
import { Task, TaskStatus, Priority } from '../types';

// 🔥 IMPORT BACKEND API CALLS
import { getAllTasks, createTask, deleteTask, updateTaskStatus } from "../api/tasks";
...
export const TaskManager: React.FC = () => {
  ...
  const loadTasks = async () => {
    try {
      const data = await getAllTasks(); // GET /tasks/all
      setTasks(data);
    } catch (err) {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    try {
      const task = await createTask(newTask);  // POST /tasks/create

      setTasks([task, ...tasks]); // prepend to UI
  ...
  const handleDeleteTask = async (id: string) => {
    try {
      await deleteTask(id); // DELETE /tasks/delete/:id
  ...
  const toggleTaskStatus = async (id: string, currentStatus: TaskStatus) => {
    const newStatus =
      currentStatus === TaskStatus.COMPLETED ? TaskStatus.TODO : TaskStatus.COMPLETED;

    try {
      await updateTaskStatus(id, newStatus);  // PUT /tasks/update/:id

      // Update frontend
      setTasks(tasks.map(t => t.id === id ? { ...t, status: newStatus, progress: newStatus === TaskStatus.COMPLETED ? 100 : 0 } : t));
            ...
            <form onSubmit={handleAddTask} className="p-4 space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-primary dark:bg-slate-700 dark:text-white"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full px-3 py-2 border dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white"
                  >
                    <option>Work</option>
...
// ====================================================================
// FOCUS MODE PAGE (NO BACKEND — UI + TIMER ONLY)
// ====================================================================
    ...
    <div className="h-full flex flex-col items-center justify-center max-w-4xl mx-auto">
      ...
      <div className="relative mb-12">
        ...
        <svg className="transform -rotate-90 w-80 h-80">
          <circle cx="160" cy="160" r={radius} stroke="currentColor" strokeWidth="12" className="text-gray-200 dark:text-slate-700" fill="transparent"/>
          <circle
            cx="160"
            cy="160"
            r={radius}
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
          <div className="text-5xl font-mono font-bold text-gray-900 dark:text-white mb-2">{formatTime(timeLeft)}</div>
      ...
      <div className="flex items-center space-x-6 mb-12">
        ...
        <button 
          onClick={resetTimer}
          className="w-12 h-12 rounded-full bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-slate-600 flex items-center justifyCenter"
        >
          <RotateCcw size={24}/>
        </button>
      </div>
